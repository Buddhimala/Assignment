...
class clientThread extends Thread {
  ...
  public void run() {
    ...
    Multithreadpoolserver newserver=new Multithreadpoolserver();
...
synchronized (this) {
      ...
      if(newserver.queue.isEmpty()==false)
      {
    	  ...
    	  clientSocket= newserver.queue.peek();
    	  for (int i = 0; i < maxClientsCount; i++) {
              if (threads[i] == null) {
                (threads[i] = new clientThread(clientSocket, Multithreadpoolserver.threads)).start();
                //PrintStream os = new PrintStream(clientSocket.getOutputStream());
                newserver.os.println("You are connected to the server. connection made from queue");
                newserver.os.close();
                break;
              }
